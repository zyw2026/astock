...
当前验证口径固定为“短线低吸因子验证”，目标是自动反推出当前状态下相对有效的策略，不是追求长期固定不变的自动策略。
...
- 自动反推候选必须先过研究验收，再允许进入运行时；运行时强调“当前状态有效”
...
5. 自动策略晋级默认优先看“当前状态有效”，长期稳定性用于保留与清退，不作为唯一晋级标准
...
- 自动反推候选进入 `runtime` 的当前口径是：
  - 长窗口 validation / replay 通过
  - 且近端 `Top5` replay 或近端 validation 至少一条通过
- 自动反推候选的生命周期分层固定为：
  - `candidate`
  - `watch`
  - `runtime`
